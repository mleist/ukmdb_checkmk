...
"""usage: ukm_ckmk [--debug ...] add_host [--help] <fqdn> [--comment=<comment>] [--set-tags=<tags>] [KEY_VALUE] ...
...
def cmd(args):
    ...
    CONTEXT = {'wato_url': settings.CFG_WATO_URL,
               'username': settings.CFG_WATO_USERNAME,
               'password': settings.CFG_WATO_PASSWORD}
    C_MK_HOST01 = ck_host.CheckMkHost(context=CONTEXT,
                                      folder='Tests',
                                      hostname='Hase')
    UKMDB_LOG.info(pformat(C_MK_HOST01.attributes))
    UKMDB_LOG.info(pformat(C_MK_HOST01.get_tags()))
